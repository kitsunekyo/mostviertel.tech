---
title: "why using useSelector might be worse than prop-drilling"
date: "2022-09-24"
...
> you might have to open the embed in codesandbox to get typescript working. for some reason it just shows `any` everywhere when embedded 🙄
...
You pretty much add conditionals and indirection whenever you want to access `user`. This bumps up cyclomatic complexity of all your components, which is a bad thing.
...
We already have our check, so lets just pass it down the subtree of `AuthenticatedApp`.
...
function UserPage({ user }: { user: User }) {
  ...
  useEffect(() => {
    // do some react-external stuff with user
    // 👍 no null-checks for user
  }, [user]);

  // 👍 only one clear return, because user cannot be null
  return (
    <div>
      <h1>hello {user.name}</h1>
      <button onClick={() => dispatch(logout())}>logout</button>
...
Like anything in development, everything comes with tradeoffs so it is on us to weigh the pros and cons and decide what works better for us.

I would pick simple, readable code over loads of `if`s any time, even if it means adding a few more props.
