...
date: 2025-08-03
excerpt:
  You ever stare at a `tsconfig.json` file, then look at your `vite.config.ts`, then glance at your `pnpm-workspace.yaml`, and think, *“Why is this import not resolving?! It worked yesterday!”*
tags:
  - javascript
  - tooling
---
...
In the browser, module resolution is extremely **explicit**:

```js
import { something } from './utils/helpers.js';
```

There’s no resolution logic, no file extension guessing, no index files unless you explicitly import them. You have to write full paths (unless you're using a bundler or [import maps](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/script/type/importmap)).

> ESModules are a pretty new thing in browsers. In the years before ESM landed, we had to bundle / concatenate all javascript into a single file. Splitting (or "chunking") was extremely hard to get right.

> That’s why tools like Vite, Webpack, and Rollup exist - they allowed us to author our code as modules long before browsers supported it. Today they *simulate* a smarter and more lenient resolution system than the actual ESM spec for the browser. (eg. some allow you to omit file extensions, or allow importing from `index.js` files)

### In Node.js

Node.js, as of 2025 (v20+ is standard), supports both CommonJS and ESM, and it resolves modules using a well-defined algorithm.
...
Also, ESM behavior is **influenced by the `type` field** in your `package.json`. If it’s `"module"`, `.js` files are treated as ESM. Otherwise, they’re CJS.

> Modules in nodejs `<12` were a whole mess. You might have stumbled over the terms `requirejs`, `amd`, `systemjs` etc. Today I would recommend you get familiar with `commonjs` (the most common legacy module system for nodejs) and `esm` (the new, standardized module system for both node and browsers). Talking about the other systems is best saved for a full-on history lesson.
...
So if you’re using esbuild directly (without Vite), and want to use `tsconfig.json` paths, you’ll need to either `bundle` your code with esbuild or reach for a plugin like [`esbuild-ts-paths`](https://github.com/aleclarson/esbuild-ts-paths).
...
**pnpm doesn’t change the resolution algorithm**, but it **exposes issues** in projects that accidentally rely on [flat hoisting](https://pnpm.io/symlinked-node-modules-structure).
